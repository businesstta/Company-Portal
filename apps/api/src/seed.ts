...
try {
  ...
  const employee = await client.query(`INSERT INTO employees(company_id,department_id,employee_no,first_name,last_name,email,position,work_location,joined_on)
    VALUES($1,$2,'EMP-0001','Kyaw','Thu','admin@company.local','HR Administrator','Yangon Office','2024-01-01') ON CONFLICT(employee_no) DO UPDATE SET email='admin@company.local',updated_at=now() RETURNING id`,[companyId,dep])
  const hash = await bcrypt.hash('Admin@123', 12)
  const user = await client.query(`INSERT INTO users(employee_id,email,password_hash,role) VALUES($1,'admin@company.local',$2,'admin') ON CONFLICT(email) DO UPDATE SET password_hash=$2 RETURNING id`,[employee.rows[0].id,hash])
  ...
  console.log('Seed complete. Admin: admin@company.local / Admin@123; Employee: mya@company.local / Employee@123')
