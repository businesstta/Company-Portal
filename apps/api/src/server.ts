...
app.get('/api/learning/courses/:id/assessment',auth,permit('Learning Management'),asyncRoute(async(req,res)=>{
  const assessmentType=z.enum(['pre_test','final']).default('final').parse(req.query.type);const progress=(await db.query(`SELECT COUNT(m.id)::int total_modules,COUNT(m.id) FILTER(WHERE stats.content_count>0 AND stats.completed_count>=stats.content_count)::int completed_modules FROM learning_modules m LEFT JOIN LATERAL(SELECT COUNT(lc.id)::int content_count,COUNT(cp.content_id)::int completed_count FROM learning_module_contents lc LEFT JOIN learning_content_progress cp ON cp.content_id=lc.id AND cp.employee_id=$2 WHERE lc.module_id=m.id) stats ON true WHERE m.course_id=$1`,[req.params.id,req.user!.employeeId])).rows[0];const unlocked=Number(progress.total_modules)>0&&Number(progress.completed_modules)===Number(progress.total_modules);if(!unlocked)return res.status(403).json({error:'Complete all modules to unlock the assessment'});if(assessmentType==='final'){const preTest=await db.query(`SELECT 1 FROM learning_assessment_attempts WHERE course_id=$1 AND employee_id=$2 AND assessment_type='pre_test' LIMIT 1`,[req.params.id,req.user!.employeeId]);if(!preTest.rowCount)return res.status(403).json({error:'Complete the Pre Test Assessment first'})}const attempts=Number((await db.query(`SELECT COUNT(*) count FROM learning_assessment_attempts WHERE course_id=$1 AND employee_id=$2 AND assessment_type=$3`,[req.params.id,req.user!.employeeId,assessmentType])).rows[0].count);if(attempts>=3)return res.status(403).json({error:'Maximum assessment attempts reached'});const questions=await db.query(`SELECT id,question_text,question_type,options,points,sequence_no FROM learning_assessment_questions WHERE course_id=$1 ORDER BY sequence_no`,[req.params.id]);res.json({questions:questions.rows,assessmentType,attemptNo:attempts+1,passingScore:80,attemptsRemaining:3-attempts})
}))

app.post('/api/learning/courses/:id/assessment/submit',auth,permit('Learning Management'),asyncRoute(async(req,res)=>{
  const input=z.object({assessmentType:z.enum(['pre_test','final']).default('final'),answers:z.record(z.string(),z.array(z.string()))}).parse(req.body);const progress=(await db.query(`SELECT COUNT(m.id)::int total_modules,COUNT(m.id) FILTER(WHERE stats.content_count>0 AND stats.completed_count>=stats.content_count)::int completed_modules FROM learning_modules m LEFT JOIN LATERAL(SELECT COUNT(lc.id)::int content_count,COUNT(cp.content_id)::int completed_count FROM learning_module_contents lc LEFT JOIN learning_content_progress cp ON cp.content_id=lc.id AND cp.employee_id=$2 WHERE lc.module_id=m.id) stats ON true WHERE m.course_id=$1`,[req.params.id,req.user!.employeeId])).rows[0];if(!Number(progress.total_modules)||Number(progress.completed_modules)!==Number(progress.total_modules))return res.status(403).json({error:'Complete all modules first'});if(input.assessmentType==='final'){const preTest=await db.query(`SELECT 1 FROM learning_assessment_attempts WHERE course_id=$1 AND employee_id=$2 AND assessment_type='pre_test' LIMIT 1`,[req.params.id,req.user!.employeeId]);if(!preTest.rowCount)return res.status(403).json({error:'Complete the Pre Test Assessment first'})}const questions=(await db.query(`SELECT id,correct_answers,points FROM learning_assessment_questions WHERE course_id=$1 ORDER BY sequence_no`,[req.params.id])).rows;if(!questions.length)return res.status(400).json({error:'This course does not have assessment questions yet'});const previous=Number((await db.query(`SELECT COUNT(*) count FROM learning_assessment_attempts WHERE course_id=$1 AND employee_id=$2 AND assessment_type=$3`,[req.params.id,req.user!.employeeId,input.assessmentType])).rows[0].count);if(previous>=3)return res.status(409).json({error:'Maximum assessment attempts reached'});const normalize=(values:unknown)=>Array.isArray(values)?values.map(String).sort():[];const total=questions.reduce((sum,q)=>sum+Number(q.points),0);const earned=questions.reduce((sum,q)=>JSON.stringify(normalize(input.answers[q.id]))===JSON.stringify(normalize(q.correct_answers))?sum+Number(q.points):sum,0);const score=Math.round(earned/total*10000)/100,passed=score>=80;const client=await db.connect();try{await client.query('BEGIN');const attempt=(await client.query(`INSERT INTO learning_assessment_attempts(course_id,employee_id,assessment_type,attempt_no,answers,score,passed) VALUES($1,$2,$3,$4,$5,$6,$7) RETURNING *`,[req.params.id,req.user!.employeeId,input.assessmentType,previous+1,input.answers,score,passed])).rows[0];let certificate=null;if(input.assessmentType==='final'&&passed){const certificateNo=`LMS-${new Date().getFullYear()}-${randomUUID().slice(0,8).toUpperCase()}`;certificate=(await client.query(`INSERT INTO learning_certificates(certificate_no,course_id,employee_id,attempt_id,score) VALUES($1,$2,$3,$4,$5) ON CONFLICT(course_id,employee_id) DO UPDATE SET attempt_id=EXCLUDED.attempt_id,score=EXCLUDED.score,status='valid' RETURNING id,certificate_no,score,issued_at,status`,[certificateNo,req.params.id,req.user!.employeeId,attempt.id,score])).rows[0]}await client.query('COMMIT');res.json({assessmentType:input.assessmentType,attemptNo:previous+1,score,passed,passingScore:80,attemptsRemaining:Math.max(0,2-previous),certificate})}catch(error){await client.query('ROLLBACK');throw error}finally{client.release()}
}))
