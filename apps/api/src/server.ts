...
const learningTrainingEventInput=z.object({title:z.string().trim().min(2).max(180),description:z.string().max(100000).optional().default(''),venue:z.string().max(250).optional().default(''),trainer:z.string().max(180).optional().default(''),startsAt:z.coerce.date(),endsAt:z.coerce.date(),capacity:z.union([z.coerce.number().int().positive(),z.null()]).optional().default(null),status:z.enum(['draft','scheduled','cancelled']).default('scheduled')}).refine(value=>value.endsAt>value.startsAt,{message:'End date and time must be after the start',path:['endsAt']})

app.get('/api/learning/training-events',auth,permit('L&D Schedule'),asyncRoute(async(req,res)=>{
  const employee=(await db.query('SELECT company_id FROM employees WHERE id=$1',[req.user!.employeeId])).rows[0];if(!employee)return res.status(404).json({error:'Employee not found'});const from=z.coerce.date().optional().parse(req.query.from),to=z.coerce.date().optional().parse(req.query.to);const result=await db.query(`SELECT id,title,description,venue,trainer,starts_at,ends_at,capacity,status FROM learning_training_events WHERE company_id=$1 AND ($2::timestamptz IS NULL OR ends_at>=$2) AND ($3::timestamptz IS NULL OR starts_at<$3) AND (status='scheduled' OR $4::boolean) ORDER BY starts_at`,[employee.company_id,from??null,to??null,['admin','hr'].includes(req.user!.role)]);res.json(result.rows)
}))

app.post('/api/learning/training-events',auth,permit('Learning Management'),asyncRoute(async(req,res)=>{
  if(!['admin','hr'].includes(req.user!.role))return res.status(403).json({error:'HR or admin access required'});const input=learningTrainingEventInput.parse(req.body);const result=await db.query(`INSERT INTO learning_training_events(company_id,title,description,venue,trainer,starts_at,ends_at,capacity,status,created_by) SELECT e.company_id,$1,$2,$3,$4,$5,$6,$7,$8,$9 FROM employees e WHERE e.id=$10 RETURNING *`,[input.title,input.description||null,input.venue||null,input.trainer||null,input.startsAt,input.endsAt,input.capacity,input.status,req.user!.id,req.user!.employeeId]);res.status(201).json(result.rows[0])
}))

app.patch('/api/learning/training-events/:id',auth,permit('Learning Management'),asyncRoute(async(req,res)=>{
  if(!['admin','hr'].includes(req.user!.role))return res.status(403).json({error:'HR or admin access required'});const input=learningTrainingEventInput.parse(req.body);const result=await db.query(`UPDATE learning_training_events event SET title=$1,description=$2,venue=$3,trainer=$4,starts_at=$5,ends_at=$6,capacity=$7,status=$8,updated_at=now() FROM employees e WHERE event.id=$9 AND e.id=$10 AND e.company_id=event.company_id RETURNING event.*`,[input.title,input.description||null,input.venue||null,input.trainer||null,input.startsAt,input.endsAt,input.capacity,input.status,req.params.id,req.user!.employeeId]);if(!result.rowCount)return res.status(404).json({error:'Training event not found'});res.json(result.rows[0])
}))
