...
app.get('/api/corporate-requests',auth,permit('Corporate'),asyncRoute(async(req,res)=>{
  const type=typeof req.query.type==='string'?req.query.type:null;const privileged=['admin','hr','manager','approver'].includes(req.user!.role);const result=await db.query(`SELECT c.*,e.employee_no,trim(e.first_name||' '||e.last_name) employee_name FROM corporate_requests c JOIN employees e ON e.id=c.employee_id WHERE ($1::text IS NULL OR c.request_type=$1) AND ($2::boolean OR c.employee_id=$3) ORDER BY c.created_at DESC`,[type,privileged,req.user!.employeeId]);res.json(result.rows)
}))

app.post('/api/corporate-requests',auth,permit('Corporate'),asyncRoute(async(req,res)=>{
  const input=z.object({requestType:z.enum(['payment','advance_clearance']),payee:z.string().max(180).optional(),purpose:z.string().min(3).max(3000),amount:z.coerce.number().nonnegative(),currency:z.string().min(3).max(10).default('MMK'),details:z.record(z.string(),z.unknown()).optional()}).parse(req.body);const approver=(await db.query(`SELECT id FROM users WHERE role IN ('manager','approver','hr','admin') AND is_active=true ORDER BY CASE role WHEN 'manager' THEN 1 WHEN 'approver' THEN 2 WHEN 'hr' THEN 3 ELSE 4 END LIMIT 1`)).rows[0];const prefix=input.requestType==='payment'?'PAY':'ADV';const reference=`${prefix}-${new Date().getFullYear()}-${Date.now().toString().slice(-7)}`;const result=await db.query(`INSERT INTO corporate_requests(employee_id,request_type,reference_no,payee,purpose,amount,currency,details,approver_id) VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *`,[req.user!.employeeId,input.requestType,reference,input.payee??null,input.purpose,input.amount,input.currency,input.details??{},approver?.id??null]);res.status(201).json(result.rows[0])
}))
