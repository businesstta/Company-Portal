...
app.post('/api/learning/modules/:id/youtube',auth,permit('Learning Management'),asyncRoute(async(req,res)=>{
  if(!['admin','hr'].includes(req.user!.role))return res.status(403).json({error:'HR or admin access required'});const input=z.object({title:z.string().trim().min(2).max(180),youtubeUrl:z.string().url().max(1000),sequenceNo:z.coerce.number().int().positive().default(1)}).parse(req.body);const videoId=youtubeVideoId(input.youtubeUrl);if(!/^[a-zA-Z0-9_-]{6,20}$/.test(videoId))return res.status(400).json({error:'Please enter a valid YouTube video URL'});const result=await db.query(`INSERT INTO learning_module_contents(module_id,content_type,title,youtube_url,youtube_video_id,sequence_no,created_by) SELECT m.id,'youtube',$1,$2,$3,$4,$5 FROM learning_modules m JOIN learning_courses c ON c.id=m.course_id JOIN employees e ON e.company_id=c.company_id WHERE m.id=$6 AND e.id=$7 RETURNING *`,[input.title,input.youtubeUrl,videoId,input.sequenceNo,req.user!.id,req.params.id,req.user!.employeeId]);if(!result.rowCount)return res.status(404).json({error:'Module not found'});res.status(201).json(result.rows[0])
}))
...
app.post('/api/learning/modules/:id/files',auth,permit('Learning Management'),learningContentUpload.single('file'),asyncRoute(async(req,res)=>{
  if(!['admin','hr'].includes(req.user!.role)){if(req.file)unlinkSync(req.file.path);return res.status(403).json({error:'HR or admin access required'})}if(!req.file)return res.status(400).json({error:'Please select a supported learning file'});const input=z.object({title:z.string().trim().min(2).max(180),sequenceNo:z.coerce.number().int().positive().default(1)}).parse(req.body);const result=await db.query(`INSERT INTO learning_module_contents(module_id,content_type,title,original_name,stored_name,mime_type,file_size,sequence_no,created_by) SELECT m.id,'file',$1,$2,$3,$4,$5,$6,$7 FROM learning_modules m JOIN learning_courses c ON c.id=m.course_id JOIN employees e ON e.company_id=c.company_id WHERE m.id=$8 AND e.id=$9 RETURNING *`,[input.title,req.file.originalname,req.file.filename,req.file.mimetype,req.file.size,input.sequenceNo,req.user!.id,req.params.id,req.user!.employeeId]);if(!result.rowCount){unlinkSync(req.file.path);return res.status(404).json({error:'Module not found'})}res.status(201).json(result.rows[0])
}))
