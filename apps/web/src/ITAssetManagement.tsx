...
const tableColumns: [string, string][] = [
  ...
  ["Purchase Price", "purchase_price"], ["Vendor/Supplier", "vendor_supplier"], ["Invoice/PO Number", "invoice_po_number"],
...
function PrintableAssetLabel({ asset, barcodeSource, qrSource }: { asset: AssetRow; barcodeSource: string; qrSource: string }) {
  ...
  return <article className="bartender-asset-label"><aside>IT ASSET</aside><header><h2>{text(asset.asset_name)}</h2><div><span>Asset Tag: {text(asset.asset_tag_code)}</span><b>{text(asset.asset_type)}</b></div></header><main><section><h3>Barcode</h3><img src={barcodeSource} alt="Barcode" /></section><section><h3>QR Code</h3><img src={qrSource} alt="QR Code" /></section></main><footer><span>{location}</span><strong>{text(asset.asset_tag_code)}</strong></footer></article>;
}

export default function ITAssetManagement({ token }: { token: string }) {
