...
type AssessmentAttempt = { attempt_no: number; score: number; passed: boolean; submitted_at: string };

type LearningRow = {
  employee_id: string; employee_no: string; employee_name: string; department: string | null; organization: string | null;
  project_location: string | null; position: string | null; course_code: string; course_title: string; course_date?: string | null; course_status: string;
  total_contents: number; completed_contents: number; progress_percentage: number; test_score: number | null; test_passed: boolean | null;
  final_attempts: number; final_attempt_history: AssessmentAttempt[]; best_score: number | null; final_pass_score: number | null;
...
const formatCourseDate = (value?: string | null) => { if (!value) return "—"; const [year, month, day] = value.slice(0, 10).split("-"); return year && month && day ? `${day}/${month}/${year}` : "—"; };
const scoreLabel = (value: number | null) => value === null ? "—" : `${Number(value)}%`;
const testStatusLabel = (row: LearningRow) => row.test_score === null ? "Not Taken" : row.test_passed ? "Passed" : "Below 80%";
const attemptHistoryLabel = (attempts: AssessmentAttempt[]) => attempts.length ? attempts.map(attempt => `#${attempt.attempt_no} ${Number(attempt.score)}% ${attempt.passed ? "Passed" : "Failed"}`).join(" · ") : "—";
...
export default function LearningDetailReport({ token }: { token: string }) {
  ...
  const detailHeaders = ["Employee ID", "Employee", "Department", "Organization", "Project Location", "Position", "Course Code", "Course", "Date", "Progress", "Test Score", "Test Status", "Final Attempts", "Final Attempt Scores", "Final Best Score", "Final Pass Score", "Status"];
  const detailRows = filtered.map(row => [row.employee_no, row.employee_name, row.department, row.organization, row.project_location, row.position, row.course_code, row.course_title, formatCourseDate(row.course_date), `${row.progress_percentage}%`, scoreLabel(row.test_score), testStatusLabel(row), row.final_attempts, attemptHistoryLabel(row.final_attempt_history ?? []), scoreLabel(row.best_score), scoreLabel(row.final_pass_score), row.certificate_earned ? "Completed · Certified" : statusLabel(row.learning_status)]);
  ...
  return <div className="learning-detail-report">
    ...
    <section className="learning-chart-grid" aria-label="Learning charts">
      <ChartCard title="Employee learning progress" value={`${employeeProgress.length} learners`} tone="purple" note="Progress changes with the selected filters" onExport={format => exportData(format, "Employee learning progress", ["Employee", "Average Progress"], employeeProgress.map(item => [item.name, `${item.value}%`]))}><VerticalBars items={employeeProgress} /></ChartCard>
      <ChartCard title="Learning by department" value={`${departmentProgress[0]?.value ?? 0}%`} tone="green" note={`${departmentProgress[0]?.name ?? "No department"} has the highest average progress`} onExport={format => exportData(format, "Learning by department", ["Department", "Average Progress"], departmentProgress.map(item => [item.name, `${item.value}%`]))}><div className="learning-pie-layout"><div className="learning-pie" style={{ background: pieGradient }}><span>{departmentProgress.length}</span></div><div className="learning-pie-legend">{departmentProgress.slice(0, 5).map((item, index) => <span key={item.name}><i style={{ background: pieColors[index] }} />{item.name}<b>{item.value}%</b></span>)}</div></div></ChartCard>
      <ChartCard title="Learning by organization" value={`${organizationProgress[0]?.value ?? 0}%`} tone="blue" note={`${organizationProgress[0]?.name ?? "No organization"} has the highest average progress`} onExport={format => exportData(format, "Learning by organization", ["Organization", "Average Progress"], organizationProgress.map(item => [item.name, `${item.value}%`]))}><HorizontalBars items={organizationProgress} /></ChartCard>
      <ChartCard title="Assessment performance" value={`${averageScore}%`} tone="orange" note={`${passed} of ${scored.length} assessed assignments passed`} onExport={format => exportData(format, "Assessment performance", ["Metric", "Value"], [["Average score", `${averageScore}%`], ["Assessed assignments", scored.length], ["Passed (80% or higher)", passed], ["Below 80%", Math.max(0, scored.length - passed)]])}><div className="assessment-scoreboard"><div><strong>{averageScore}%</strong><span>Average score</span></div><i><b style={{ width: `${averageScore}%` }} /></i><section><span><b>{passed}</b>Passed</span><span><b>{Math.max(0, scored.length - passed)}</b>Below 80%</span></section></div></ChartCard>
    </section>
    ...
    <section className="learning-report-table"><header><div><small>EMPLOYEE DATA</small><h2>Learning Management details</h2></div><div className="learning-table-actions"><span>{filtered.length.toLocaleString()} records</span><ExportActions label="Learning Management details" onExport={format => exportData(format, "Learning Management details", detailHeaders, detailRows)} /></div></header><div className="learning-table-scroll"><table><thead><tr><th>Employee ID</th><th>Employee</th><th>Department</th><th>Organization</th><th>Project Location</th><th>Position</th><th>Course</th><th>Date</th><th>Progress</th><th>Test Score</th><th>Test Status</th><th>Final Attempts</th><th>Final Attempt Scores</th><th>Final Best Score</th><th>Final Pass Score</th><th>Status</th></tr></thead><tbody>{pageRows.map(row => <tr key={`${row.employee_id}-${row.course_code}`}><td><b>{row.employee_no}</b></td><td>{row.employee_name}</td><td>{row.department ?? "—"}</td><td>{row.organization ?? "—"}</td><td>{row.project_location ?? "—"}</td><td>{row.position ?? "—"}</td><td><b>{row.course_title}</b><small>{row.course_code}</small></td><td>{formatCourseDate(row.course_date)}</td><td><div className="learning-progress"><span><i style={{ width: `${row.progress_percentage}%` }} /></span><b>{row.progress_percentage}%</b></div></td><td>{scoreLabel(row.test_score)}</td><td><span className={`assessment-result ${row.test_score === null ? "not-taken" : row.test_passed ? "passed" : "failed"}`}>{testStatusLabel(row)}</span></td><td>{row.final_attempts}</td><td><div className="assessment-attempts" title={attemptHistoryLabel(row.final_attempt_history ?? [])}>{(row.final_attempt_history ?? []).map(attempt => <span className={attempt.passed ? "passed" : "failed"} key={attempt.attempt_no}>#{attempt.attempt_no} · {Number(attempt.score)}%</span>)}{!(row.final_attempt_history ?? []).length && "—"}</div></td><td>{scoreLabel(row.best_score)}</td><td><b className="final-pass-score">{scoreLabel(row.final_pass_score)}</b></td><td><span className={`learning-status ${row.learning_status}`}>{row.certificate_earned ? "Completed · Certified" : statusLabel(row.learning_status)}</span></td></tr>)}</tbody></table></div>
...
function ChartCard({ title, value, tone, note, children, onExport }: { title: string; value: string; tone: string; note: string; children: ReactNode; onExport: (format: "excel" | "pdf") => void }) {
  return <article className={`learning-chart-card ${tone}`}><header><div><small>OVERVIEW</small><h2>{title}</h2></div><div className="learning-chart-actions"><b>{value}</b><ExportActions label={title} onExport={onExport} compact /></div></header><div className="learning-chart-visual">{children}</div><p>{note}</p></article>;
}

function ExportActions({ label, onExport, compact = false }: { label: string; onExport: (format: "excel" | "pdf") => void; compact?: boolean }) {
  return <div className={`learning-export-actions${compact ? " compact" : ""}`} aria-label={`Export ${label}`}><button type="button" onClick={() => onExport("excel")} title={`Export ${label} to Excel`}>Excel</button><button type="button" onClick={() => onExport("pdf")} title={`Export ${label} to PDF`}>PDF</button></div>;
}
