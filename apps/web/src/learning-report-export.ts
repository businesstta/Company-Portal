...
export function exportPdf(title: string, headers: string[], rows: ExportCell[][]) {
  ...
  const streams = pages.map((pageRows, pageIndex) => {
    const commands = [rect(0, height - 72, width, 72, "0.027 0.106 0.31"), rect(margin, height - 26, 44, 4, "1 0.796 0.212"), text("COMPANY PORTAL", margin, height - 45, 9, 180, true, "1 1 1"), text(title, margin, height - 63, 18, 520, true, "1 1 1"), text(`L&D DETAIL REPORT  |  ${rows.length.toLocaleString()} RECORDS`, width - 265, height - 47, 8, 235, true, "0.86 0.89 0.96"), text(`Generated ${new Date().toLocaleString()}`, width - 265, height - 61, 7, 235, false, "0.75 0.8 0.9"), text("Learning Management reporting export", margin, sectionLabelY, 9, 400, true), text(`Page ${pageIndex + 1} of ${pages.length}`, width - 110, sectionLabelY, 8, 80, true, "0.32 0.27 0.72")];
