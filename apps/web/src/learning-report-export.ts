...
export async function exportExcel(title: string, headers: string[], rows: ExportCell[][], token: string, endpoint = "/reports/learning-export") {
  const response = await fetch(`${API}${endpoint}`, { method: "POST", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" }, body: JSON.stringify({ title, headers, rows }) });
...
export function exportPdf(title: string, headers: string[], rows: ExportCell[][], reportLabel = "L&D DETAIL REPORT", sectionLabel = "Learning Management reporting export") {
  const width = 842, height = 595, margin = 28, tableWidth = width - margin * 2, detail = headers.length > 6, rowHeight = detail ? 17 : 23, sectionLabelY = 501, tableTop = 463, rowsPerPage = Math.max(1, Math.floor((tableTop - 48) / rowHeight));
  ...
  const streams = pages.map((pageRows, pageIndex) => {
    const commands = [rect(0, height - 72, width, 72, "0.027 0.106 0.31"), rect(margin, height - 26, 44, 4, "1 0.796 0.212"), text("COMPANY PORTAL", margin, height - 45, 9, 180, true, "1 1 1"), text(title, margin, height - 63, 18, 520, true, "1 1 1"), text(`${reportLabel}  |  ${rows.length.toLocaleString()} RECORDS`, width - 265, height - 47, 8, 235, true, "0.86 0.89 0.96"), text(`Generated ${new Date().toLocaleString()}`, width - 265, height - 61, 7, 235, false, "0.75 0.8 0.9"), text(sectionLabel, margin, sectionLabelY, 9, 400, true), text(`Page ${pageIndex + 1} of ${pages.length}`, width - 110, sectionLabelY, 8, 80, true, "0.32 0.27 0.72")];
