...
function DataPage({
  ...
  const [clearanceAdvanceAmount,setClearanceAdvanceAmount]=useState(0);
  const [clearanceAdvanceAmountText,setClearanceAdvanceAmountText]=useState("");
  const [clearanceActualCost,setClearanceActualCost]=useState(0);
  const [clearanceActualCostText,setClearanceActualCostText]=useState("");
  ...
  const createCorporate = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if(isAdvanceClearance){const advance=eligibleAdvances.find(item=>String(item.id)===String(f.get("paymentRequestId")));const advanceAmount=Number(f.get("advanceAmount")??0),actualCost=Number(f.get("actualCost")??0),balance=advanceAmount-actualCost;f.set("requestType","advance_clearance");f.set("payee",String(advance?.reference_no??""));f.set("purpose",String(f.get("remark")??"Advance clearance"));f.set("amount",String(advanceAmount));f.set("currency",String(advance?.currency??"MMK"));f.set("details",JSON.stringify({businessUnit:f.get("businessUnit"),dbVrNo:f.get("dbVrNo"),paymentRequestId:f.get("paymentRequestId"),paymentRequestReference:advance?.reference_no,advanceAmount,actualCost,balance,total:balance,remark:f.get("remark")}))}
    if (isVehicleRequest) {
        ...
        {showForm && (
          ...
          <form ref={paymentRequestFormRef} className="employee-form payment-request-form advance-clearance-form" onSubmit={createCorporate}>
            ...
            <fieldset><legend>Clearance information</legend><div className="form-grid">
              <label className="wide">Payment Request ID *<select name="paymentRequestId" value={selectedAdvanceId} onChange={event=>{const id=event.target.value,advance=eligibleAdvances.find(item=>String(item.id)===id),amount=Number(advance?.amount??0);setSelectedAdvanceId(id);setClearanceAdvanceAmount(amount);setClearanceAdvanceAmountText(id?amount.toLocaleString("en-US"):"");setClearanceActualCost(0);setClearanceActualCostText("")}} required><option value="">Select approved Advance Payment Request</option>{eligibleAdvances.map(item=><option key={String(item.id)} value={String(item.id)}>{String(item.reference_no)} · {Number(item.amount).toLocaleString()} {String(item.currency)}</option>)}</select><small>{eligibleAdvances.length?"Only your approved and uncleared Advance requests are shown.":"No approved Advance Payment Request is available for clearance."}</small></label>
              {(()=>{const advance=eligibleAdvances.find(item=>String(item.id)===selectedAdvanceId),balance=clearanceAdvanceAmount-clearanceActualCost,currency=String(advance?.currency??"MMK"),updateAmount=(raw:string,setNumber:(value:number)=>void,setText:(value:string)=>void)=>{const clean=raw.replace(/[^0-9.]/g,"");const[whole="",...decimalParts]=clean.split(".");const decimal=decimalParts.join("").slice(0,2),numeric=Number(`${whole||0}${decimalParts.length?`.${decimal}`:""}`)||0;setNumber(numeric);setText(`${whole?Number(whole).toLocaleString("en-US"):""}${decimalParts.length?`.${decimal}`:""}`)};return <><label>Advance Amount *<input type="text" inputMode="decimal" value={clearanceAdvanceAmountText} onChange={event=>updateAmount(event.target.value,setClearanceAdvanceAmount,setClearanceAdvanceAmountText)} required disabled={!advance}/><input type="hidden" name="advanceAmount" value={clearanceAdvanceAmount}/></label><label>Actual Cost *<input type="text" inputMode="decimal" value={clearanceActualCostText} onChange={event=>updateAmount(event.target.value,setClearanceActualCost,setClearanceActualCostText)} required disabled={!advance}/><input type="hidden" name="actualCost" value={clearanceActualCost}/></label><label>Balance<input value={`${balance.toLocaleString("en-US")} ${currency}`} readOnly/></label><label>Total<input value={`${balance.toLocaleString("en-US")} ${currency}`} readOnly/></label></>})()}
              <label className="wide">Remark *<textarea name="remark" rows={4} required/></label>
              <label className="wide payment-attachments">Attachments<input name="attachments" type="file" multiple/><small>Up to 20 files, maximum 500 MB each. Documents, spreadsheets, images, audio, video and archive files are supported.</small></label>
            </div></fieldset>
            <div className="payment-approval-note"><b>Approval workflow</b><span>Department Head Approver → Finance Approver → Cashier → Receiver</span><small>Payment Request ID is linked to the selected approved Advance request.</small></div>
