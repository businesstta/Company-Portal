...
function App() {
  ...
  const login=async(e:FormEvent<HTMLFormElement>)=>{e.preventDefault();const f=new FormData(e.currentTarget);const r=await fetch(`${API}/auth/login`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email:f.get('email'),password:f.get('password')})});if(!r.ok){setLoginError('Invalid login');return}const data=await r.json();localStorage.setItem('mobile_token',data.token);setToken(data.token)}
  ...
  if(!token)return <div className="mobile-login"><form onSubmit={login}><div>CP</div><small>COMPANY PORTAL</small><h1>Employee sign in</h1><p>Attendance and self-service portal</p><label>Email<input name="email" type="email" defaultValue="mya@company.local" required/></label><label>Password<input name="password" type="password" defaultValue="Employee@123" required/></label>{loginError&&<span>{loginError}</span>}<button>Sign in →</button></form></div>
